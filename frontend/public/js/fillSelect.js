...
(() => {
	...
	const setCountriesLists = () => {
		countriesLists.forEach((select, i) => {
			select.innerHTML = `<option value="" selected disabled hidden>-- Sélectionnez un pays --</option>`;
	...
	const setStatesList = (i, countryISO) => {
		...
		statesLists[i].innerHTML = `<option value="" selected disabled hidden>-- Sélectionnez une région --</option>`;
		if (countryISO) {
	...
	const setCitiesList = (citiesList, countryISO, stateISO) => {
		...
		citiesList.innerHTML = `<option value="" selected disabled hidden>-- Sélectionnez une ville --</option>`;
		if (stateISO) {
